  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border rounded-lg p-6">
        ...
        {/* 关键指标（紧凑版：label/value 同行，避免“大字居中八宫格”那种廉价感） */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 mt-6 pt-6 border-t">
          <div className="flex items-center justify-between gap-3 rounded-md border border-slate-800 bg-slate-900/30 px-3 py-2">
            <div className="text-[11px] text-muted-foreground">MA200</div>
            <div className="text-sm sm:text-base font-mono font-medium flex items-center gap-2">
              <span>{ma200 ? ma200.toFixed(2) : "-"}</span>
              {ma200 && (
                <span className={`text-[11px] ${priceVsMa200 >= 0 ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <div className="flex items-center justify-between gap-3 rounded-md border border-slate-800 bg-slate-900/30 px-3 py-2">
            <div className="text-[11px] text-muted-foreground">52W High</div>
            <div className="text-sm sm:text-base font-mono font-medium">{high52w.toFixed(2)}</div>
          </div>

          <div className="flex items-center justify-between gap-3 rounded-md border border-slate-800 bg-slate-900/30 px-3 py-2">
            <div className="text-[11px] text-muted-foreground">52W Low</div>
            <div className="text-sm sm:text-base font-mono font-medium">{low52w.toFixed(2)}</div>
          </div>

          <div className="flex items-center justify-between gap-3 rounded-md border border-slate-800 bg-slate-900/30 px-3 py-2">
            <div className="text-[11px] text-muted-foreground">Return 1M</div>
            <div className={`text-sm sm:text-base font-mono font-medium ${return1M !== null ? (return1M >= 0 ? 'text-green-600' : 'text-red-600') : 'text-muted-foreground'}`}>
              {return1M !== null ? `${return1M >= 0 ? '+' : ''}${return1M.toFixed(1)}%` : '-'}
            </div>
          </div>

          <div className="flex items-center justify-between gap-3 rounded-md border border-slate-800 bg-slate-900/30 px-3 py-2">
            <div className="text-[11px] text-muted-foreground">Return 3M</div>
            <div className={`text-sm sm:text-base font-mono font-medium ${return3M !== null ? (return3M >= 0 ? 'text-green-600' : 'text-red-600') : 'text-muted-foreground'}`}>
              {return3M !== null ? `${return3M >= 0 ? '+' : ''}${return3M.toFixed(1)}%` : '-'}
            </div>
          </div>

          <div className="flex items-center justify-between gap-3 rounded-md border border-slate-800 bg-slate-900/30 px-3 py-2">
            <div className="text-[11px] text-muted-foreground">Return 1Y</div>
            <div className={`text-sm sm:text-base font-mono font-medium ${return1Y !== null ? (return1Y >= 0 ? 'text-green-600' : 'text-red-600') : 'text-muted-foreground'}`}>
          ...
          <div className="flex items-center justify-between gap-3 rounded-md border border-slate-800 bg-slate-900/30 px-3 py-2">
            ...
            <div className="text-sm sm:text-base font-mono font-medium text-red-500">
          ...
          <div className="flex items-center justify-between gap-3 rounded-md border border-slate-800 bg-slate-900/30 px-3 py-2">
            ...
            <div className="text-sm sm:text-base font-mono font-medium">{(quote.volume / 1000000).toFixed(1)}M</div>
          </div>
        </div>

        {/* 元信息 */}
        <div className="grid grid-cols-2 gap-2 mt-2">
          <div className="flex items-center justify-between gap-3 rounded-md border border-slate-800 bg-slate-900/20 px-3 py-2">
            <div className="text-[11px] text-muted-foreground">Source</div>
            <div className="text-xs sm:text-sm font-mono font-medium truncate" title={quote.source}>{quote.source}</div>
          </div>
          <div className="flex items-center justify-between gap-3 rounded-md border border-slate-800 bg-slate-900/20 px-3 py-2">
